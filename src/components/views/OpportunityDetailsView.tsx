...
export const OpportunityDetailsView: React.FC = () => {
  ...
  const [liveMatchedSkills, setLiveMatchedSkills] = useState<string[] | null>(null);
  const [liveSkillGaps, setLiveSkillGaps] = useState<string[] | null>(null);

  const handleLiveMatch = async () => {
    setIsMatching(true);
    try {
      const res = await runOpportunityMatch(opp.id);
      if (res) {
        setLiveExplanation(res.explanation);
        if (res.matchedSkills.length > 0) setLiveMatchedSkills(res.matchedSkills);
        if (res.skillGaps.length > 0) setLiveSkillGaps(res.skillGaps);
      }
    } finally {
      setIsMatching(false);
    }
    ...
    <div className="p-6 lg:p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-8">
            ...
            <div className="p-4 rounded-xl bg-blue-950/20 border border-blue-500/30 space-y-2.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-xs font-bold text-blue-300">
                  ...
                  <span>Explainable Match Breakdown (Gemini)</span>
                </div>
                <button
                  onClick={handleLiveMatch}
                  disabled={isMatching}
                  className="px-2.5 py-1 rounded-lg bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/30 text-[10px] font-bold text-blue-300 flex items-center gap-1 transition-colors disabled:opacity-60"
                >
                  {isMatching ? (
                    <>
              ...
              <blockquote className="text-xs text-slate-200 leading-relaxed italic border-l-2 border-blue-500 pl-3">
                &ldquo;{liveExplanation || opp.aiExplanation}&rdquo;
              </blockquote>
            </div>
          </section>
          ...
          {(() => {
            const currentMatched = liveMatchedSkills || opp.matchedSkills;
            const currentMissing = liveSkillGaps || opp.missingSkills;
            return (
              <section className="p-6 rounded-2xl bg-[#0F172A] border border-slate-800 space-y-5 shadow-sm">
                <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                  ...
                  <span className="text-xs text-slate-400">
                    {currentMatched.length} of {opp.skills.length} skills verified
                  </span>
                </div>
